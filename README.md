...
- `/` は月間カレンダー形式の日付選択ページです。
- 日付ごとにセッション件数と AI credit 合計を表示します（例: `3件 / 12.4 AIU`）。
- `0件` の日付はグレーアウトされ、クリックできません。
- 日付を選択すると `/?date=YYYY-MM-DD` へ遷移し、その日のセッション一覧を表示します。
- 月の切替は `/?month=YYYY-MM` で指定できます（前月/翌月リンクも提供）。
...
- セッションの日付帰属は `main.jsonl`（元ログ）の最終更新日時（mtime）を基準とします。
